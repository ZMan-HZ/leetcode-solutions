 ...
 * 提示：</p>
 * <p>
 * 0 <= word1.length, word2.length <= 500<p>
...
public class MinimumEditDistance {
     ...
     * 你可能觉得 horse 到 ro 这个问题也很难解决。
     * 但是没关系，我们可以继续用上面的方法拆分这个问题，对于这个问题拆分出来的所有子问题，我们也可以继续拆分，直到：
     ...
     * @return
     */
    public static int minEditionDistance(String word1, String word2) {
        ...
        //动态规划矩阵
        //dp[i][j]:表示word1字符的前i个字符和word2的前j个字符之间的编辑距离
        ...
        for (int index = 1; index < length1 + 1; index++) {
            for (int jndex = 1; jndex < length2 + 1; jndex++) {
                int left = dp[index - 1][jndex] + 1;
                int right = dp[index][jndex - 1] + 1;
                int mid = dp[index - 1][jndex - 1];
                if (word1.charAt(index - 1) != word2.charAt(jndex - 1)) {
                    mid++;
                }
                dp[index][jndex] = Math.min(left, Math.min(right, mid));
            }
        }
    ...
    public static void main(String[] args) {
        String word1 = "horse";
        String word2 = "ros";
        int distance = minEditionDistance(word1, word2);
        System.out.println("distance = " + distance);

        word1 = "intention";
        word2 = "execution";
        distance = minEditionDistance(word1, word2);
        System.out.println("distance = " + distance);
