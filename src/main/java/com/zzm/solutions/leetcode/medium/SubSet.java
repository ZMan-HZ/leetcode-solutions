 ...
 * </blockquote>
 *
 * <p>示例 1： 输入：nums = [1,2,3] 输出：[[],[1],[2],[1,2],[3],[1,3],[2,3],[1,2,3]]
 *
...
public class SubSet {
  ...
  private static void combine(
      int[] nums, int[] subset, int start, int counter, List<List<Integer>> results) {
    if (counter == 0) {
  ...
  /**
   * 字典排序法 思路：只有对应位置为1时的坐标取对应位置的元素 比如nums的长度为3的【1，2，3】 则有字典组合如： 000 ---> 没有1，则时[]空集 001 ---->
   * 取值的元素[3] 010 ----> 取值的元素[2] 100 ----> 取值的元素[1] 011 ---> 取值的元素[2，3] 101 ----> 取值的元素[1，3] 110
   * ----> 取值的元素[1，2] 111 ----> 取值的元素[1，2，3] 此时所有子集取完
   *
   * @param nums
   * @return
   */
  ...
  private static int trans(String input) {
    ...
    for (Character str : input.toCharArray()) {
      if (map.containsKey(str)) {
        ...
        int weight = (int) Math.pow(16, index--);
        Integer integer = Integer.valueOf(str + "");
        number += weight * integer;
      }
    }
    return number;
  }
